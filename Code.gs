...
function _sheet6_legend(ss) {
  ...
  var items = [
    ...
    { type: ROW, a: '🚦 מדד ארצי',
      b: 'שקופית של מספר אחד: מדד התנועה הארצי — ממוצע משוקלל (לפי מס\' פקקים) של "סטייה %" מול ההיסטוריה, לחלון שעות השיא שנסגר (בוקר 06–10 / ערב 16–19). כולל טבלת מדד לפי חלון, מגמת הקריאות האחרונות עם sparkline, ואזהרת "מדגם קטן" כשהחלון דל. מתעדכן אוטומטית ~10:00 ו-~20:00 (או ידנית: "🚦 חשב מדד ארצי עכשיו"), ובסוף כל ריצה נשלח מייל תקציר. פירוט מלא ב-§9.' },

    { type: ROW, a: 'פירוט לפי מסגרת זמן',
      b: 'הפירוט שמאחורי המדד הארצי. אותה השוואה כמו "פירוט לפי שעה", אבל השעות מקובצות למסגרות (בוקר/ערב). שורה לכל מסלול × כיוון בכל מסגרת, עם שורת סיכום שמציגה את המדד המשוקלל, מס\' הפקקים והמסלולים של המסגרת. בלתי תלוי בפילטר — תמיד התאריך האחרון מול הארכיון הקבוע.' },
    ...
    { type: ROW, a: 'period (תקופה)', b: 'חירום או שגרה — מסווג בעת ה-upload לפי זמן הפקק מול חלונות חירום מוגדרים בקוד (EMERGENCY_WINDOWS). לסיווג מחדש של נתונים ישנים: מתקדם ← "🔴 סווג חירום/שגרה לנתונים קיימים".' },

    { type: BLANK },
    { type: SEC, text: '§3. ארכיטקטורת הנתונים — סוגי לשוניות' },
    { type: HDR, a: 'לשונית', b: 'מתי מתעדכנת ומה היא צוברת' },

    { type: ROW, a: '🔄 9 לשוניות הניתוח',
      b: 'לוח מחוונים · סיכום מסלולים · פירוט לפי שעה · השוואת כיוונים · חריגות · פירוט פקקים · מקרא ומתודולוגיה · 🚦 מדד ארצי · פירוט לפי מסגרת זמן — **נמחקות ונבנות מחדש בכל "החל סינון"**. הפילטר חל על raw_data, אבל ההשוואות מול _baseline_archive (כל ההיסטוריה). שתי לשוניות המדד בלתי תלויות בפילטר (תמיד החלון האחרון מול כל ההיסטוריה) ומתעדכנות גם בתזמון הבוקר/ערב.' },

    { type: ROW, a: '🔁 אגרגציה לאורך זמן',
      b: 'נבנית מחדש **רק בעת upload חדש**, לא בעת סינון. תצוגה של _baseline_archive ממוין מהחדש לישן.' },

    { type: ROW, a: '📥 raw_data',
      b: 'גלוי. שורה לכל פקק בכל snapshot — מקור גרעיני. עמודות: snapshot_ts, pub_ts, date, day, hour, tbin, street, city, level, length_m, delay_s, speed_kmh, start_node, end_node, tt_min, route_name, dir_ix, archived, period. **נשמר 30 יום ואז עובר prune אוטומטי** (השורות הישנות מיוצאות ל-Drive כ-CSV לפני המחיקה; בנוסף תקרת שורות בטיחותית מפעילה קציצה מוקדמת לפני מגבלת התאים של Sheets).' },
    ...
    { type: ROW, a: '🔒 _nci_history (מוסתר)',
      b: 'יומן קבוע של כל קריאת מדד ארצי — run_ts, date, window, daytype, index_pct, n_jams, n_routes, period. **לעולם לא נמחק** — מזין את המגמה בשקופית המדד ובמייל.' },

    { type: ROW, a: '🔒 _archive_slice_log (מוסתר)',
      b: 'יומן אבחון של ייצוא ה-CSV המקוטע — שורה לכל slice (מס\' chunks, זמנים, סיבת יציאה). לכוונון ביצועי הייצוא בלבד.' },
    ...
    { type: ROW, a: 'החרגת חירום', b: 'שעות שבתוך חלון חירום (EMERGENCY_WINDOWS) מוחרגות מחישוב ה-baseline — הסטייה נמדדת תמיד מול תנועת שגרה בלבד.' },
    ...
    { type: BLANK },
    { type: SEC, text: '§9. מדד התנועה הארצי (NCI)' },
    { type: ROW, a: 'מה זה', b: 'מספר אחד למצב הארצי: ממוצע משוקלל (לפי מס\' פקקים) של "סטייה %" של כל המסלולים מול ה-baseline ההיסטורי, בחלון שעות השיא האחרון שנסגר. חיובי = כבד מהשגרה, שלילי = קל.' },
    { type: ROW, a: 'חלונות ותזמון', b: 'בוקר 06:00–10:00 (מחושב ~10:00) · ערב 16:00–19:00 (מחושב ~20:00). כל קריאה נרשמת ל-_nci_history. אפשר גם ידנית: "🚦 חשב מדד ארצי עכשיו".' },
    { type: ROW, a: 'מהימנות', b: 'פחות מ-80 פקקים או פחות מ-12 מסלולים בחלון → אזהרת "מדגם קטן" ליד המספר (נפוץ בסופ"ש, שבו התנועה דלה מטבעה). האזהרה לא מסתירה את המספר — רק מסייגת.' },
    { type: ROW, a: 'מייל תקציר', b: 'אחרי כל חישוב נשלח מייל עם המדד, 5 המסלולים הכבדים, מגמה אחרונה, תמונת מפת חום אזורית (דורש מפתח Geoapify חינמי) וקישור למפה אינטראקטיבית. הפעלה/כיבוי, נמענים ובדיקה — מתפריט 🚦 Waze.' },
    { type: ROW, a: 'מפת חום', b: 'שלושה אזורים (צפון/מרכז/דרום — לפי המחוזות הרשמיים, מקובצים) צבועים לפי הסטייה האזורית המשוקללת של אותו חלון. גרסה אינטראקטיבית (Leaflet) מוגשת כ-Web App, וגרסת תמונה סטטית משובצת במייל.' },

    { type: BLANK },
    { type: SEC, text: '§10. סולם צבעים בעמודות סטייה' },
    ...
    { type: BLANK },
    { type: SEC, text: '§11. מגבלות המתודולוגיה' },
    ...
    { type: BLANK },
    { type: SEC, text: '§12. רענון הנתונים' },
    ...
    { type: ROW, a: 'גיבוי יומי', b: 'כל יום ב-03:00: ייצוא אינקרמנטלי ל-Drive + קציצה. ניתן לכיבוי: "🗓️ הפעל/כבה גיבוי יומי". ייצוא גדול רץ מקוטע ברקע עם חידוש אוטומטי אם נתקע.' },
    { type: ROW, a: 'מדד ארצי', b: 'מחושב אוטומטית פעמיים ביום (~10:00, ~20:00), נרשם ל-_nci_history ונשלח במייל.' },
    { type: ROW, a: 'מיגרציה', b: 'תפריט "🚦 Waze → 🔄 העבר נתונים קיימים לארכיון" — קולט שורות ישנות שעדיין לא נספרו לארכיון.' },

    { type: BLANK },
    { type: SEC, text: '§13. מילון מונחים מקוצר' },
